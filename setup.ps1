...
if (!(Test-Path ".\rel_openbor_rc1.zip"))
{
	curl.exe -LO https://github.com/steward-fu/archives/releases/download/miyoo-mini-plus/rel_openbor_rc1.zip
}
...
# if (!(Test-Path ".\bin_parasyte_20220125.zip"))
# {
# 	curl.exe -LO https://github.com/steward-fu/archives/releases/download/miyoo-mini/bin_parasyte_20220125.zip
# }

echo "extract sdl lib(miyoo mini plus)"
if (!(Test-Path ".\openbor"))
{
	Expand-Archive -Path .\rel_openbor_rc1.zip -DestinationPath .\
}

echo "extract sdl lib(windows)"
...
Copy-Item -Path .\openbor\libs\libSDL2-2.0.so.0 -Destination $env:USERPROFILE\.rustup\toolchains\stable-x86_64-pc-windows-msvc\lib\rustlib\armv7-unknown-linux-gnueabihf\lib\libSDL2.so -Force
...
echo "you need to add the following config:"
echo "### ### ### ### ### ### ### ### ### ###"
cat .\config
echo "### ### ### ### ### ### ### ### ### ###"
cd ..
